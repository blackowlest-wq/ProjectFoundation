...
- [ ] **Step 1: 受入条件とBDDケースを正本ケースへ記録する**

`日報承認差戻し_テストケース.md`へ、TC-APR-001～010を正常系、異常系、権限、状態、コメント境界、DB副作用、API、UI、E2Eの観測可能な期待結果付きで記録する。各ケースに受入条件ID、テスト層、実テストID欄、対象外・保留欄を付ける。承認・差し戻し後の詳細APIに、状態だけでなく監査項目が返ることを期待結果へ含める。

- [ ] **Step 2: テスト構成・責務対応表を作成する**

既存テストと追加テストを、機能、ユースケース、コンポーネント、テスト層、本番対象、ケースID、実テストID、分割・維持判断、理由で対応付ける。承認・差し戻しAPIは新規`DailyReportApprovalControllerTest`、未承認一覧・詳細UIは新規`DailyReportApprovalPanel.test.tsx`、業務フローは新規`approval-rejection.spec.ts`へ分離し、既存の提出・検索・編集テストは回帰用として維持する。

- [ ] **Step 3: ケースレビューと受入条件レビューを完了する**

`日報承認差戻し_受入条件レビュー.md`へ、仕様根拠、期待結果、権限、状態遷移、DB整合性、CSRF、E2E、対象外・保留を記録する。`日報承認差戻し_テストケースレビュー.md`へ、テスト不足、テスト構成、期待結果の判定を記録する。仕様にない期待値は追加せず、必要なものは確認事項または保留とする。

- [ ] **Step 4: 実装前作業記録を作成する**

`日報承認差戻し_作業記録.md`へ、分離worktree、既存変更、参照資料、外部ドキュメント確認結果、採用した設計、対象外、仮定、保留、次のDoで守ることを記録する。実装前のP0レビュー入力、TDD対象、静的解析・coverage・Oracle実行可否を記載する。

- [ ] **Step 5: コミットする**

```powershell
git add docs/AI活用開発研究/作業記録/日報承認差戻し_作業記録.md docs/AI活用開発研究/作業記録/日報承認差戻し_受入条件レビュー.md docs/AI活用開発研究/作業記録/日報承認差戻し_テストケース.md docs/AI活用開発研究/作業記録/日報承認差戻し_テストケースレビュー.md
git commit -m "docs: define approval and rejection test cases"
```
...
- Modify: `docs/AI活用開発研究/作業記録/日報承認差戻し_テストケースレビュー.md`
...
- [ ] **Step 1: 実装結果を正本ケースと作業記録へ反映する**

Task 0で確定した`TC-APR-*`へ実テストID、実行コマンド、結果、証跡を追記する。`日報承認差戻し_受入条件レビュー.md`へ実装後の対応状況、未充足条件、保留条件を追記し、`日報承認差戻し_作業記録.md`へ変更ファイルと実行結果を記録する。
...
- Task 0でF-005/F-007/F-008/F-010、A-009/A-012/A-013/A-015、S-005/S-006/S-007の実装前ケースとレビューを完了し、Task 1～5で実装する。
