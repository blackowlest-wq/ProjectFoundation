...
- [x] 品質ツールの依存関係とバージョンを固定する。
- [x] Frontend lint、Markdown lint、Backend Spotless/Checkstyle/SpotBugsが単独で呼び出せるようにする。
- [x] 生成物、Oracle設定、ログ、ツールキャッシュをGitleaksとGit差分から除外する。
...
- [x] `scripts/check.ps1 -Mode Quick`がstaged差分の空白、生成物、lint、Spotless、秘密情報を確認する。
- [x] `scripts/check.ps1 -Mode Full`がFrontendとBackendの品質検査を順番に実行し、全失敗を集約して非0終了する。
- [x] `scripts/doctor.ps1`がPowerShell、Node/npm、Java、Maven、Lefthook、Gitleaks、Playwrightを確認する。
- [x] `lefthook.yml`がpre-commitをQuick、pre-pushをFullへ接続する。
- [x] Lefthook設定または実行ツールがない場合に成功扱いにならないことを確認する。
...
- [x] Oracle接続情報をローカル設定または保護された環境変数から読み取る。
- [x] DDL実行を明示許可なしで拒否する。
- [x] Oracle安全ガードテストで危険な設定を検出する。
...
- [x] Pull Requestとmain pushでQuality Workflowを実行する。
- [x] 週次・手動でSecurity Workflowを実行する。
- [x] main push・手動でOracle Workflowを実行する。
- [x] Workflow内のNode/Java/PowerShell/ツールバージョンとローカルRunnerの設定を一致させる。
...
- [x] 標準実行入口、Quick/Full/CIの役割、Oracle安全条件を記録する。
- [x] 今回の空振り原因と対応を指摘一覧へ記録する。

### Task 6: 品質ゲートを実行して確認する

- [x] `pwsh -NoProfile -File scripts/doctor.ps1`を実行し、環境差異を確認する。
- [x] `pwsh -NoProfile -File scripts/check.ps1 -Mode Quick`を実行する。
- [x] `pwsh -NoProfile -File scripts/check.ps1 -Mode Full`を実行する。
- [x] `pwsh -NoProfile -File scripts/check.ps1 -CiTask E2E`を実行する。
- [ ] `pwsh -NoProfile -File scripts/check.ps1 -CiTask BackendCoverage`をOracle共通入口で実行する。
- [x] `git diff --check`、テスト配置チェック、Hook設定確認を実行する。
- [x] 失敗があれば原因を修正し、結果を作業記録へ記載する。

> BackendCoverageは、Git管理外のOracle設定に`DAILY_REPORT_DB_ENV`と期待識別値が不足しているため、接続前ガードで停止した。設定値を推測・補完せず、専用runnerまたは保護された環境変数が揃った環境で再実行する。
