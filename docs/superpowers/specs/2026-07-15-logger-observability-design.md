...
- ログは`ERROR`で、例外オブジェクトを渡してスタックトレースを残す。
...
9. Backendログ、Playwright HTMLレポート、失敗時のtraceを成果物として回収する。
...
- 想定外例外が共通500レスポンスになり、ログにはスタックトレース、レスポンスには内部情報がない。
...
`E2EOracle`はOracle接続情報とテストデータを扱うため、既存のOracle workflowへ追加し、BackendログとPlaywrightレポートを`always()`条件でアップロードする。生成ログ、coverage、Playwright成果物、Oracle設定はGitへ追加しない。
