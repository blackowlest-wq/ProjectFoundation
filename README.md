...
2. 対象機能の設計書と受入条件を確認し、[実装前チェック表](docs/AI活用開発研究/構想メモ/標準化/実装前チェック表.md)で不明点や仕様の不足を解消する。
...
7. [実装後レビュー表](docs/AI活用開発研究/構想メモ/標準化/実装後レビュー表.md)でAIの自己レビューと専門観点レビューを実施し、指摘を修正または保留判断する。レビューで得られた再発防止に有効な指摘は、個別ケースだけでなく標準の観点へ反映するか検討する。
8. [テスト・静的解析チェック表](docs/AI活用開発研究/構想メモ/標準化/テスト・静的解析チェック表.md)と[品質ゲート運用](docs/AI活用開発研究/構想メモ/標準化/品質ゲート運用.md)に従って検証し、結果を作業記録へ残します。

AIを使わずに実装する場合も、2〜8の実装前確認、テスト設計、レビュー、品質ゲート、記録の流れを適用します。実装前・実装後のフェーズ手順と品質ゲートはリポジトリのProjectFoundation Skill、観点別レビューの定義は[AI専門レビュー用プロンプト定義](docs/AI活用開発研究/構想メモ/標準化/AI専門レビュー用プロンプト定義.md)で管理します。Skillと専門プロンプトは通常、利用者が直接編集・実行する資料ではありません。
...
flowchart TD
    ...
    PREFLIGHT_SKILL["projectfoundation-preflight-ja Skill<br/>実装前ゲート"]
    REVIEW_SKILL["projectfoundation-review-ja Skill<br/>実装後ゲート"]
    PRECHECK["実装前確認・チェック"]
    TEST["BDD・TDD・テスト設計"]
    IMPLEMENT["実装"]
    ...
    DESIGN --> REQUEST
    REQUEST --> SPECIALIST
    REQUEST --> PREFLIGHT_SKILL
    REQUEST --> REVIEW_SKILL
    FLOW --> PRECHECK
    FLOW --> TEST
    FLOW --> REVIEW
    FLOW --> QUALITY
...
- AI内部用: AI専門レビュー用プロンプト定義、標準化資料内のAI用Skill。通常の利用者は直接編集しない。
